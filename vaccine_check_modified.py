...
for i in range(100000):   # Enter a suitable number of times you want the program to check
    now=datetime.datetime.now()
    print("**Checking for slots** ","["+now.strftime("%d-%m-%Y, %H:%M:%S")+"]")
    # READING RECIEVER LISTS
    df=pd.read_excel("recievers.xlsx")  # Insert the file path for the recievers folder here
    err_101=0
    num_of_days_to_be_checked=14        # Insert number of days to be checked here
        ...
        if flag==1:
            ...
            if not(row["mail"]=="no"):
                try:
                    ...
                    s.starttls()
                    s.login("vaccinecheckkannur@gmail.com","password")  #replace this with your mail ID and password. (sample mail ID, password given)  
                except:
            ...
            if flagw==1:                     # Whatsapp Web automation code
                try:
